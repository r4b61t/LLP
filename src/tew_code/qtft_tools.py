...
from new_basis_llp_qaoa.statevector_sim.statevector_llp import StateVectorLLP, get_str, qubits_number_for_action
...
class DC_QAOA():
    ...
    def optimized(self, maxiter=20, method='COBYLA'):
        ...
        for i in tqdm(range(len(self.groups)), desc="Performing QAOA for subgroups"):
            ...
            m = StateVectorLLP(Q, A, self.e, self.p)
    ...
    def _combine_groups(self, L, R, ps, q_number, action_bit_len):
        ...
        m2 = StateVectorLLP(self.Qs[LR.nodes], self.As[LR.nodes][:, LR.nodes], self.e, self.p)
        for x in LR.xs:
